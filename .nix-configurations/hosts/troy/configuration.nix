...

{
  ...
  programs.xss-lock = let
    lock = pkgs.writeShellScriptBin "lock" ''
      XSECURELOCK_PASSWORD_PROMPT=time_hex XSECURELOCK_FONT='JetBrainsMono Nerd Font' ${pkgs.xsecurelock}/bin/xsecurelock
    '';
  in {
    enable = true;
  ...
  in {
    ...
    profiles = {
      "mobile" = {
        fingerprint = { eDP-1 = eDP1.fingerprint; };
        config = { eDP-1 = lib.mkMerge [ eDP1.config { position = "0x0"; } ]; };
      };
      ...
      "home" = {
        ...
        config = {
          eDP-1 = lib.mkMerge [ eDP1.config { position = "0x768"; } ];
